...
func (gears Gears) FindGear(point Point) *Gear {
...
func (g *Gear) AddNumber(number Number) {
...
func (n *Number) GetNestedGears() []Point {
	var points []Point
...
func main() {
	...
	for _, number := range numbers {
		nestedGears := number.GetNestedGears()
		for _, point := range nestedGears {
			gear := gears.FindGear(point)
			if gear == nil {
				gear = &Gear{
					Point: point,
				}
	...
	for _, gear := range gears {
		...
		for _, number := range gear.Numbers {
			result = result * number.ToInt()
		}
		results += result
	}
